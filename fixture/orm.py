...
class ORMFixture:
    ...
    def __init__(self, host, name, user, password):    # описание привязки к базе данных
        self.db.bind('mysql', host=host, database=name, user=user, password=password, conv=decoders)  # параметр conv используется для выбора конвектаров
    ...
    # каждый блок кода, который будет взаимодействовать с базой данных должен быть помочен (что он должен выполняться в рамках сессии)
    @db_session
